...
for j, n in enumerate([1, 10, 50]):
    for i in range(1, 21):
        averages[i - 1], _ = averaged_seeds([result.epoch(i) for result in results], corpus='test', n=n)
    pyplot.plot(np.array(range(1, 21)), averages, label='%d seeds' % n, color=colors[j])

for i in range(1, 21):
    averages[i - 1], stds[i - 1] = \
        averaged_seeds([result.epoch(i) for result in results], corpus='test', n=100)
pyplot.errorbar(np.array(range(1, 21)), averages, yerr=stds, label="100 seeds", color=(1.0, 0.0, 0.0))
